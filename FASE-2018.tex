...
\begin{abstract}
In this paper, we propose a change-based approach for persisting models that conform to object-oriented (MOF/Ecore) metamodelling architectures. We demonstrate how change-based persistence can preserve fine-grained changes to models and a possible extension to enabled high-performance incremental model processing (e.g. transformation, validation) through fast model change detection. We illustrate a language-independent change-based model representation format, and an optimised model loading algorithm that seeks to avoid replaying changes that have no impact on the eventual state of the model. We report on benchmarks that compare model loading and saving performance of the proposed change-based representation against the standard state-based representation format (XMI). Our results show considerable savings in terms of persisting and identifying changes made to models at an increased---but linear---model loading cost.
\end{abstract}

\section{Introduction}
\label{sec:introduction}
In model-based software engineering in large complex systems, most existing model persistence approaches are state-based. This results in developers working on different snapshots of models. Futhermore, activities, such as model comparison and change detection, are computationally consuming. Having faster model comparison and precise difference detection can be beneficial, not only in the context of collaborative modelling where developers need to differentiate models and merge changes frequently, but also for incremental model processing \cite{rath2012derived}, \cite{ogunyomi2015property}. 

Instead of working on snapshots of models, we are experimenting with a change-based approach that persists changes made to models. The change-based approach comes with a number of potential benefits over stated-based persistence, such as the ability to detect changes much faster and more precisely, which then have positive knock-on effects in helping developers to differentiate models in collaborative modelling as well as supporting faster incremental model validation and transformation. Change-based persistence comes with a downside---an increased load cost---since we need to replay all the changes of a model to arrive at its eventual state.   

Therefore, we are proposing an optimised loading algorithm. We extends the previous work of Yohannis et al. \cite{yohannis2017turning} that implements a change-based persistence (CBP) format, by proposing and evaluating an algorithm to improve the loading time of change-based models that avoids replaying changes that have no impact on the eventual state of the model (e.g. that are cancelled out by subsequent changes). We asses the efficiency of the algorithm in terms of persisting changes, loading time, and memory consumption. Our approach demonstrate considerable saving in time performance for persisting and identifying changes made to models at an increased, but linear, model loading cost, compared to state-based models when tested on large scale models.
